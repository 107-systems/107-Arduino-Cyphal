...
// Generated at:  2023-01-29 16:46:55.801325 UTC
// Is deprecated: yes
// Fixed port-ID: 405
...
struct Response_0_1 final
{
    ...
    serialize(nunavut::support::bitspan out_buffer) const
    {
        ...
        {   // truncated uint40 size
            ...
            const auto _result3_ = out_buffer.setUxx(size, 40U);
            if(not _result3_){
        ...
        {   // truncated uint40 unix_timestamp_of_last_modification
            NUNAVUT_ASSERT(out_buffer.offset_alings_to_byte());
            NUNAVUT_ASSERT(40ULL <= out_buffer.size());
            const auto _result3_ = out_buffer.setUxx(unix_timestamp_of_last_modification, 40U);
            if(not _result3_){
                return -_result3_.error();
            }
            out_buffer.add_offset(40U);
        ...
        {   // saturated bool is_file_not_directory
            ...
            auto _result2_ = out_buffer.setBit(is_file_not_directory);
            if(not _result2_){
        ...
        {   // saturated bool is_link
            NUNAVUT_ASSERT(1ULL <= out_buffer.size());
            auto _result2_ = out_buffer.setBit(is_link);
            if(not _result2_){
                return -_result2_.error();
            }
            out_buffer.add_offset(1UL);
        ...
        {   // saturated bool is_readable
            NUNAVUT_ASSERT(1ULL <= out_buffer.size());
            auto _result2_ = out_buffer.setBit(is_readable);
            if(not _result2_){
                return -_result2_.error();
            }
            out_buffer.add_offset(1UL);
        ...
        {   // saturated bool is_writeable
            NUNAVUT_ASSERT(1ULL <= out_buffer.size());
            auto _result2_ = out_buffer.setBit(is_writeable);
            if(not _result2_){
                return -_result2_.error();
            }
            out_buffer.add_offset(1UL);
    ...
    deserialize(nunavut::support::const_bitspan in_buffer)
    {
        ...
        // truncated uint40 size
        NUNAVUT_ASSERT(in_buffer.offset_alings_to_byte());
        size = in_buffer.getU64(40U);
        in_buffer.add_offset(40U);
        // truncated uint40 unix_timestamp_of_last_modification
        NUNAVUT_ASSERT(in_buffer.offset_alings_to_byte());
        unix_timestamp_of_last_modification = in_buffer.getU64(40U);
        in_buffer.add_offset(40U);
        // saturated bool is_file_not_directory
        NUNAVUT_ASSERT(in_buffer.offset_alings_to_byte());
        is_file_not_directory = in_buffer.getBit();
        in_buffer.add_offset(1U);
        // saturated bool is_link
        is_link = in_buffer.getBit();
        in_buffer.add_offset(1U);
        // saturated bool is_readable
        is_readable = in_buffer.getBit();
        in_buffer.add_offset(1U);
        // saturated bool is_writeable
        is_writeable = in_buffer.getBit();
        in_buffer.add_offset(1U);
        // void4
