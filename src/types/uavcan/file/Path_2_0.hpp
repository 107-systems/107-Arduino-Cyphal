...
// Generated at:  2023-03-10 05:17:53.514369 UTC
// Is deprecated: no
// Fixed port-ID: None
...
static_assert( nunavut::support::options::target_endianness == 1693710260,
              "/tmp/public_regulated_data_types/uavcan/file/Path.2.0.dsdl "
...
struct Path_2_0 final
{
    struct _traits_  // The name is surrounded with underscores to avoid collisions with DSDL attributes.
    {
        ...
        {
            TypeOf() = delete;
            using path = nunavut::support::VariableLengthArray<std::uint8_t, 255>;
        };
    };
    ...
    static constexpr std::uint8_t MAX_LENGTH = 255U;
...
inline nunavut::support::SerializeResult serialize(const Path_2_0& obj,
                                                   nunavut::support::bitspan out_buffer)
{
    ...
    {   // saturated uint8[<=255] path
        if (obj.path.size() > 255)
        {
            return -nunavut::support::Error::SerializationBadArrayLength;
        }
        // Array length prefix: truncated uint8
        const auto _result3_ = out_buffer.setUxx(obj.path.size(), 8U);
        if(not _result3_){
            return -_result3_.error();
        }
        out_buffer.add_offset(8U);
        for (std::size_t _index2_ = 0U; _index2_ < obj.path.size(); ++_index2_)
        {
            // Saturation code not emitted -- native representation matches the serialized representation.
            const auto _result3_ = out_buffer.setUxx(obj.path[_index2_], 8U);
            if(not _result3_){
                return -_result3_.error();
            }
            out_buffer.add_offset(8U);
...
inline nunavut::support::SerializeResult deserialize(Path_2_0& obj,
                                                     nunavut::support::const_bitspan in_buffer)
{
    ...
    in_buffer.add_offset(8U);
        ...
        for (std::size_t _index5_ = 0U; _index5_ < _size0_; ++_index5_)
        {
            ...
            obj.path.push_back();
            obj.path[_index5_] = in_buffer.getU8(8U);
        in_buffer.add_offset(8U);
        }
    }
...
#endif // UAVCAN_FILE_PATH_2_0_HPP_INCLUDED
