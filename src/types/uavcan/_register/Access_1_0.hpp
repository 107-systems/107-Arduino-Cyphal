...
// Generated at:  2023-01-18 06:06:50.377449 UTC
// Is deprecated: no
// Fixed port-ID: 384
...
struct Request_1_0 final
{
    ...
    static constexpr std::size_t EXTENT_BYTES                    =515UL;
    static constexpr std::size_t SERIALIZATION_BUFFER_SIZE_BYTES =515UL;
    ...
    ///
    uavcan::_register::Value_1_0 value;
    ...
    serialize(nunavut::support::bitspan out_buffer) const
    {
        ...
        {   // uavcan.register.Value.1.0 value
            NUNAVUT_ASSERT(out_buffer.offset_alings_to(8U));
            NUNAVUT_ASSERT(out_buffer.offset_alings_to_byte());
            NUNAVUT_ASSERT(2072ULL <= out_buffer.size());
            ...
            // It is assumed that we know the exact type of the serialized entity, hence we expect the size to match.
            NUNAVUT_ASSERT((_size_bytes0_ * 8U) >= 8ULL);
            NUNAVUT_ASSERT((_size_bytes0_ * 8U) <= 2072ULL);
            out_buffer.add_offset(_size_bytes0_ * 8U);
            // NUNAVUT_ASSERT(out_buffer.size() >= 0);
        }
        {
        ...
        NUNAVUT_ASSERT(out_buffer.offset() <= 4120ULL);
        NUNAVUT_ASSERT(out_buffer.offset_alings_to_byte());
...
struct Response_1_0 final
{
    ...
    static constexpr std::size_t EXTENT_BYTES                    =267UL;
    static constexpr std::size_t SERIALIZATION_BUFFER_SIZE_BYTES =267UL;
    static_assert(EXTENT_BYTES >= SERIALIZATION_BUFFER_SIZE_BYTES, "Internal constraint violation");
    static_assert(EXTENT_BYTES < (std::numeric_limits<std::size_t>::max() /8U), "This message is too large to be handled by current types!");
    ...
    serialize(nunavut::support::bitspan out_buffer) const
    {
        ...
        {   // saturated bool mutable
            ...
            auto _result2_ = out_buffer.setBit(_mutable);
            if(not _result2_){
        ...
        {   // saturated bool persistent
            NUNAVUT_ASSERT(1ULL <= out_buffer.size());
            auto _result2_ = out_buffer.setBit(persistent);
            if(not _result2_){
                return -_result2_.error();
            }
            out_buffer.add_offset(1UL);
        ...
        NUNAVUT_ASSERT(out_buffer.offset() <= 2136ULL);
        NUNAVUT_ASSERT(out_buffer.offset_alings_to_byte());
        return out_buffer.offset_bytes_ceil();
    }
    ...
    deserialize(nunavut::support::const_bitspan in_buffer)
    {
        ...
        // saturated bool mutable
        NUNAVUT_ASSERT(in_buffer.offset_alings_to_byte());
        _mutable = in_buffer.getBit();
        in_buffer.add_offset(1U);
        // saturated bool persistent
        persistent = in_buffer.getBit();
        in_buffer.add_offset(1U);
        // void6
