...
// Generated at:  2023-01-18 06:06:50.694311 UTC
// Is deprecated: no
// Fixed port-ID: None
...
struct Frame_0_1 final
{
    ...
    static constexpr std::size_t EXTENT_BYTES                    =9232UL;
    static constexpr std::size_t SERIALIZATION_BUFFER_SIZE_BYTES =9232UL;
    ...
    serialize(nunavut::support::bitspan out_buffer) const
    {
        ...
        {   // saturated uint8[6] destination
            ...
            for (size_t _index1_ = 0U; _index1_ < 6UL; ++_index1_)
            {
                ...
                const auto _result3_ = out_buffer.setUxx(destination[_index1_], 8U);
                if(not _result3_){
        ...
        {   // saturated uint8[6] source
            ...
            for (size_t _index1_ = 0U; _index1_ < 6UL; ++_index1_)
            {
                ...
                NUNAVUT_ASSERT(8ULL <= out_buffer.size());
                // Saturation code not emitted -- native representation matches the serialized representation.
                const auto _result3_ = out_buffer.setUxx(source[_index1_], 8U);
                if(not _result3_){
                    return -_result3_.error();
                }
                out_buffer.add_offset(8U);
        ...
        {   // uavcan.metatransport.ethernet.EtherType.0.1 ethertype
            ...
            auto _err0_ = ethertype.serialize(_subspan0_.value());
            if (not _err0_)
            {
                return _err0_;
            }
        ...
        {   // saturated uint8[<=9216] payload
            NUNAVUT_ASSERT(out_buffer.offset_alings_to_byte());
            NUNAVUT_ASSERT(73744ULL <= out_buffer.size());
            if (payload.size() > 9216)
            {
        ...
        NUNAVUT_ASSERT(out_buffer.offset() <= 73856ULL);
        NUNAVUT_ASSERT(out_buffer.offset_alings_to_byte());
    ...
    deserialize(nunavut::support::const_bitspan in_buffer)
    {
        ...
        {
            NUNAVUT_ASSERT(in_buffer.offset_alings_to_byte());
            destination[_index4_] = in_buffer.getU8(8U);
        ...
        {
            NUNAVUT_ASSERT(in_buffer.offset_alings_to_byte());
            source[_index4_] = in_buffer.getU8(8U);
            in_buffer.add_offset(8U);
        }
        ...
        NUNAVUT_ASSERT(in_buffer.offset_alings_to_byte());
        {
            ...
            NUNAVUT_ASSERT(in_buffer.offset_alings_to_byte());
            {
                const auto _err1_ = ethertype.deserialize(in_buffer.subspan());
                if(_err1_){
