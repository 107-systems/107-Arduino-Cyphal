...
// Generated at:  2024-01-12 03:09:34.805937 UTC
// Is deprecated: yes
// Fixed port-ID: None
...
///
struct [[deprecated("uavcan.metatransport.udp.Endpoint.0.1 is reaching the end of its life; there may be a newer version available")]]Endpoint_0_1 final
{
    struct _traits_  // The name is surrounded with underscores to avoid collisions with DSDL attributes.
    {
        ...
        {
            TypeOf() = delete;
            using ip_address = std::array<std::uint8_t,16>;
    ...
    ///
    _traits_::TypeOf::ip_address ip_address{};
    ///
    /// MAC address of the host in the network byte order (big endian).
    ///
    _traits_::TypeOf::mac_address mac_address{};
    ///
    /// The UDP port number.
    ///
    _traits_::TypeOf::port port{};
};

inline nunavut::support::SerializeResult serialize(const Endpoint_0_1& obj,
                                                   nunavut::support::bitspan out_buffer)
{
    ...
    {   // saturated uint8[16] ip_address
        ...
        for (std::size_t _index1_ = 0U; _index1_ < 16UL; ++_index1_)
        {
            ...
            const auto _result3_ = out_buffer.setUxx(obj.ip_address[_index1_], 8U);
            if(not _result3_){
    ...
    {   // saturated uint8[6] mac_address
        ...
        {
            // Saturation code not emitted -- native representation matches the serialized representation.
            const auto _result3_ = out_buffer.setUxx(obj.mac_address[_index1_], 8U);
            if(not _result3_){
                return -_result3_.error();
            }
            out_buffer.add_offset(8U);
    ...
    {   // saturated uint16 port
        // Saturation code not emitted -- native representation matches the serialized representation.
        const auto _result3_ = out_buffer.setUxx(obj.port, 16U);
        if(not _result3_){
            return -_result3_.error();
        }
...
inline nunavut::support::SerializeResult deserialize(Endpoint_0_1& obj,
                                                     nunavut::support::const_bitspan in_buffer)
{
    ...
    {
        obj.ip_address[_index4_] = in_buffer.getU8(8U);
        in_buffer.add_offset(8U);
    }
    // saturated uint8[6] mac_address
    for (std::size_t _index4_ = 0U; _index4_ < 6UL; ++_index4_)
    {
        obj.mac_address[_index4_] = in_buffer.getU8(8U);
        in_buffer.add_offset(8U);
    }
    // saturated uint16 port
    obj.port = in_buffer.getU16(16U);
