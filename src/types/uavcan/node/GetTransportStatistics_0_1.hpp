...
// Generated at:  2023-03-10 05:17:53.846936 UTC
// Is deprecated: no
// Fixed port-ID: 434
...
struct Response_0_1 final
{
    struct _traits_  // The name is surrounded with underscores to avoid collisions with DSDL attributes.
    {
        ...
        static constexpr std::size_t SerializationBufferSizeBytes = 61UL;
        static_assert(ExtentBytes >= SerializationBufferSizeBytes, "Internal constraint violation");
        static_assert(ExtentBytes < (std::numeric_limits<std::size_t>::max() / 8U), "This message is too large to be handled by the selected types");
        struct TypeOf
        {
            TypeOf() = delete;
            using transfer_statistics = uavcan::node::IOStatistics_0_1;
            using network_interface_statistics = nunavut::support::VariableLengthArray<uavcan::node::IOStatistics_0_1, 3>;
        };
    };
    ...
    ///
    _traits_::TypeOf::transfer_statistics transfer_statistics;
    ///
    ...
    ///
    _traits_::TypeOf::network_interface_statistics network_interface_statistics{};
};
                                                   ...
                                                   nunavut::support::bitspan out_buffer)
{
    ...
    {   // uavcan.node.IOStatistics.0.1 transfer_statistics
        ...
        auto _err0_ = serialize(obj.transfer_statistics, _subspan0_.value());
        if (not _err0_)
        {
            return _err0_;
        }
    ...
    {   // uavcan.node.IOStatistics.0.1[<=3] network_interface_statistics
        if (obj.network_interface_statistics.size() > 3)
        {
            return -nunavut::support::Error::SerializationBadArrayLength;
        }
        // Array length prefix: truncated uint8
        const auto _result3_ = out_buffer.setUxx(obj.network_interface_statistics.size(), 8U);
        if(not _result3_){
            return -_result3_.error();
        }
        out_buffer.add_offset(8U);
        for (std::size_t _index2_ = 0U; _index2_ < obj.network_interface_statistics.size(); ++_index2_)
        {
            std::size_t _size_bytes0_ = 15UL;  // Nested object (max) size, in bytes.
            auto _subspan0_ = out_buffer.subspan(0U, _size_bytes0_ * 8U);
            if(not _subspan0_){
            ...
            auto _err0_ = serialize(obj.network_interface_statistics[_index2_], _subspan0_.value());
            if (not _err0_)
            {
                return _err0_;
            }
                                                     ...
                                                     nunavut::support::const_bitspan in_buffer)
{
    ...
    // uavcan.node.IOStatistics.0.1 transfer_statistics
    {
        ...
        {
            const auto _err1_ = deserialize(obj.transfer_statistics, in_buffer.subspan());
            if(_err1_){
    ...
    in_buffer.add_offset(8U);
        ...
        obj.network_interface_statistics.reserve(_size0_);
        for (std::size_t _index5_ = 0U; _index5_ < _size0_; ++_index5_)
        {
            ...
            obj.network_interface_statistics.push_back();
            {
            std::size_t _size_bytes1_ = in_buffer.size() / 8U;
            {
                const auto _err1_ = deserialize(obj.network_interface_statistics[_index5_], in_buffer.subspan());
                if(_err1_){
                    _size_bytes1_ = _err1_.value();
                }else{
                    return -_err1_.error();
...
struct GetTransportStatistics_0_1
{
    GetTransportStatistics_0_1() = delete;

    using Request  = GetTransportStatistics::Request_0_1;
    using Response = GetTransportStatistics::Response_0_1;
