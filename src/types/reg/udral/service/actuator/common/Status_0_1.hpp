...
// Generated at:  2023-01-24 05:52:12.201867 UTC
// Is deprecated: no
// Fixed port-ID: None
...
struct Status_0_1 final
{
    ...
    static constexpr std::size_t SERIALIZATION_BUFFER_SIZE_BYTES =14UL;
    ...
    reg::udral::service::actuator::common::FaultFlags_0_1 fault_flags;
    ...
    serialize(nunavut::support::bitspan out_buffer) const
    {
        ...
        {   // uavcan.si.unit.temperature.Scalar.1.0 motor_temperature
            ...
            auto _err0_ = motor_temperature.serialize(_subspan0_.value());
            if (not _err0_)
            {
                return _err0_;
            }
        ...
        {   // uavcan.si.unit.temperature.Scalar.1.0 controller_temperature
            ...
            NUNAVUT_ASSERT(_subspan0_->offset_alings_to_byte());
            auto _err0_ = controller_temperature.serialize(_subspan0_.value());
            if (not _err0_)
            {
                return _err0_;
            }
        ...
        {   // reg.udral.service.actuator.common.FaultFlags.0.1 fault_flags
            ...
            NUNAVUT_ASSERT(_subspan0_->offset_alings_to_byte());
            auto _err0_ = fault_flags.serialize(_subspan0_.value());
            if (not _err0_)
            {
                return _err0_;
            }
        ...
        // It is assumed that we know the exact type of the serialized entity, hence we expect the size to match.
        NUNAVUT_ASSERT(out_buffer.offset() == 112ULL);
        NUNAVUT_ASSERT(out_buffer.offset_alings_to_byte());
    ...
    deserialize(nunavut::support::const_bitspan in_buffer)
    {
        ...
        NUNAVUT_ASSERT(in_buffer.offset_alings_to_byte());
        {
            ...
            NUNAVUT_ASSERT(in_buffer.offset_alings_to_byte());
            {
                const auto _err1_ = motor_temperature.deserialize(in_buffer.subspan());
                if(_err1_){
        ...
        NUNAVUT_ASSERT(in_buffer.offset_alings_to_byte());
        {
            ...
            NUNAVUT_ASSERT(in_buffer.offset_alings_to_byte());
            {
                const auto _err1_ = controller_temperature.deserialize(in_buffer.subspan());
                if(_err1_){
                    _size_bytes1_ = _err1_.value();
                }else{
                    return -_err1_.error();
        ...
        NUNAVUT_ASSERT(in_buffer.offset_alings_to_byte());
        {
            ...
            NUNAVUT_ASSERT(in_buffer.offset_alings_to_byte());
            {
                const auto _err1_ = fault_flags.deserialize(in_buffer.subspan());
                if(_err1_){
                    _size_bytes1_ = _err1_.value();
                }else{
                    return -_err1_.error();
