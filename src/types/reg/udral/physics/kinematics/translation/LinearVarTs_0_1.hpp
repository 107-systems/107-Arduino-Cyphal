...
// Generated at:  2023-01-18 06:06:51.569373 UTC
// Is deprecated: no
// Fixed port-ID: None
...
#include "reg/udral/physics/kinematics/translation/LinearTs_0_1.hpp"
...
struct LinearVarTs_0_1 final
{
    ...
    static constexpr std::size_t EXTENT_BYTES                    =25UL;
    static constexpr std::size_t SERIALIZATION_BUFFER_SIZE_BYTES =25UL;
    ...
    reg::udral::physics::kinematics::translation::LinearTs_0_1 value;
    ///
    /// [meter^2]
    ///
    ...
    serialize(nunavut::support::bitspan out_buffer) const
    {
        ...
        {   // saturated float16 position_error_variance
            ...
            float _sat1_ = position_error_variance;
        ...
        {   // saturated float16 velocity_error_variance
            NUNAVUT_ASSERT(out_buffer.offset_alings_to_byte());
            NUNAVUT_ASSERT(16ULL <= out_buffer.size());
            float _sat1_ = velocity_error_variance;
            if (std::isfinite(_sat1_))
            {
                if (_sat1_ < static_cast<float>(-65504.0))
                {
        ...
        {   // saturated float16 acceleration_error_variance
            NUNAVUT_ASSERT(out_buffer.offset_alings_to_byte());
            NUNAVUT_ASSERT(16ULL <= out_buffer.size());
            float _sat1_ = acceleration_error_variance;
            if (std::isfinite(_sat1_))
            {
                if (_sat1_ < static_cast<float>(-65504.0))
                {
        ...
        // It is assumed that we know the exact type of the serialized entity, hence we expect the size to match.
        NUNAVUT_ASSERT(out_buffer.offset() == 200ULL);
        NUNAVUT_ASSERT(out_buffer.offset_alings_to_byte());
    ...
    deserialize(nunavut::support::const_bitspan in_buffer)
    {
        ...
        // saturated float16 position_error_variance
        NUNAVUT_ASSERT(in_buffer.offset_alings_to_byte());
        position_error_variance = in_buffer.getF16();
        in_buffer.add_offset(16U);
        // saturated float16 velocity_error_variance
        NUNAVUT_ASSERT(in_buffer.offset_alings_to_byte());
        velocity_error_variance = in_buffer.getF16();
        in_buffer.add_offset(16U);
        // saturated float16 acceleration_error_variance
        NUNAVUT_ASSERT(in_buffer.offset_alings_to_byte());
        acceleration_error_variance = in_buffer.getF16();
        in_buffer.add_offset(16U);
