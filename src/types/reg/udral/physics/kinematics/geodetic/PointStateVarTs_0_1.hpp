...
// Generated at:  2023-01-24 05:52:12.312642 UTC
// Is deprecated: no
// Fixed port-ID: None
...
struct PointStateVarTs_0_1 final
{
    ...
    static constexpr std::size_t EXTENT_BYTES                    =67UL;
    static constexpr std::size_t SERIALIZATION_BUFFER_SIZE_BYTES =67UL;
    ...
    reg::udral::physics::kinematics::geodetic::PointStateVar_0_1 value;
    ...
    serialize(nunavut::support::bitspan out_buffer) const
    {
        ...
        {   // reg.udral.physics.kinematics.geodetic.PointStateVar.0.1 value
            NUNAVUT_ASSERT(out_buffer.offset_alings_to(8U));
            NUNAVUT_ASSERT(out_buffer.offset_alings_to_byte());
            NUNAVUT_ASSERT(480ULL <= out_buffer.size());
            ...
            _size_bytes0_ = _err0_.value();
            // It is assumed that we know the exact type of the serialized entity, hence we expect the size to match.
            NUNAVUT_ASSERT((_size_bytes0_ * 8U) == 480ULL);
            out_buffer.add_offset(_size_bytes0_ * 8U);
            // NUNAVUT_ASSERT(out_buffer.size() >= 0);
        }
        {
        ...
        // It is assumed that we know the exact type of the serialized entity, hence we expect the size to match.
        NUNAVUT_ASSERT(out_buffer.offset() == 536ULL);
        NUNAVUT_ASSERT(out_buffer.offset_alings_to_byte());
