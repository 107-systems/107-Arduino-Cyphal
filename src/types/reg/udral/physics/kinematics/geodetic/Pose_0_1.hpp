...
// Generated at:  2023-01-18 06:06:51.644616 UTC
// Is deprecated: no
// Fixed port-ID: None
...
#include "reg/udral/physics/kinematics/geodetic/Point_0_1.hpp"
...
struct Pose_0_1 final
{
    ...
    static constexpr std::size_t EXTENT_BYTES                    =40UL;
    static constexpr std::size_t SERIALIZATION_BUFFER_SIZE_BYTES =40UL;
    ...
    reg::udral::physics::kinematics::geodetic::Point_0_1 position;
    ...
    serialize(nunavut::support::bitspan out_buffer) const
    {
        ...
        {   // uavcan.si.unit.angle.Quaternion.1.0 orientation
            ...
            NUNAVUT_ASSERT(_subspan0_->offset_alings_to_byte());
            auto _err0_ = orientation.serialize(_subspan0_.value());
            if (not _err0_)
            {
                return _err0_;
            }
            _size_bytes0_ = _err0_.value();
            // It is assumed that we know the exact type of the serialized entity, hence we expect the size to match.
            NUNAVUT_ASSERT((_size_bytes0_ * 8U) == 128ULL);
            out_buffer.add_offset(_size_bytes0_ * 8U);
            // NUNAVUT_ASSERT(out_buffer.size() >= 0);
        }
        {
        ...
        // It is assumed that we know the exact type of the serialized entity, hence we expect the size to match.
        NUNAVUT_ASSERT(out_buffer.offset() == 320ULL);
        NUNAVUT_ASSERT(out_buffer.offset_alings_to_byte());
    ...
    deserialize(nunavut::support::const_bitspan in_buffer)
    {
        ...
        NUNAVUT_ASSERT(in_buffer.offset_alings_to_byte());
        {
            ...
            NUNAVUT_ASSERT(in_buffer.offset_alings_to_byte());
            {
                const auto _err1_ = orientation.deserialize(in_buffer.subspan());
                if(_err1_){
                    _size_bytes1_ = _err1_.value();
                }else{
                    return -_err1_.error();
