...
// Generated at:  2023-01-29 16:46:56.590855 UTC
// Is deprecated: no
// Fixed port-ID: None
...
struct PressureTempVarTs_0_1 final
{
    ...
    static constexpr std::size_t EXTENT_BYTES                    =21UL;
    static constexpr std::size_t SERIALIZATION_BUFFER_SIZE_BYTES =21UL;
    ...
    ///   2 -- kelvin^2
    ///
    std::array<float,3> covariance_urt{};
    ...
    serialize(nunavut::support::bitspan out_buffer) const
    {
        ...
        {   // uavcan.si.unit.pressure.Scalar.1.0 pressure
            ...
            NUNAVUT_ASSERT(_subspan0_->offset_alings_to_byte());
            auto _err0_ = pressure.serialize(_subspan0_.value());
            if (not _err0_)
            {
                return _err0_;
            }
        ...
        {   // uavcan.si.unit.temperature.Scalar.1.0 temperature
            ...
            NUNAVUT_ASSERT(_subspan0_->offset_alings_to_byte());
            auto _err0_ = temperature.serialize(_subspan0_.value());
            if (not _err0_)
            {
                return _err0_;
            }
        ...
        // It is assumed that we know the exact type of the serialized entity, hence we expect the size to match.
        NUNAVUT_ASSERT(out_buffer.offset() == 168ULL);
        NUNAVUT_ASSERT(out_buffer.offset_alings_to_byte());
    ...
    deserialize(nunavut::support::const_bitspan in_buffer)
    {
        ...
        NUNAVUT_ASSERT(in_buffer.offset_alings_to_byte());
        {
            ...
            NUNAVUT_ASSERT(in_buffer.offset_alings_to_byte());
            {
                const auto _err1_ = pressure.deserialize(in_buffer.subspan());
                if(_err1_){
                    _size_bytes1_ = _err1_.value();
                }else{
                    return -_err1_.error();
        ...
        NUNAVUT_ASSERT(in_buffer.offset_alings_to_byte());
        {
            ...
            NUNAVUT_ASSERT(in_buffer.offset_alings_to_byte());
            {
                const auto _err1_ = temperature.deserialize(in_buffer.subspan());
                if(_err1_){
                    _size_bytes1_ = _err1_.value();
                }else{
                    return -_err1_.error();
