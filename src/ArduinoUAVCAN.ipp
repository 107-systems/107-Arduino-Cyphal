...
int8_t ArduinoUAVCAN::publish(T_MSG const & msg)
{
  ...
  if (!publish(CANARD_NODE_ID_UNSET, CanardTransferKindMessage, T_MSG::PORT_ID, payload_size, payload_buf, &transfer_id))
    return ERROR;
  else
    return transfer_id;
}
...
int8_t ArduinoUAVCAN::request(T_REQ const & req, CanardNodeID const remote_node_id, OnTransferReceivedFunc func)
{
